...
# - MWAA webserver host (only when MWAA exists)
...
locals {
  ...
  mwaa_webserver_url = try(data.terraform_remote_state.mwaa.outputs.mwaa_webserver_url, null)

  mwaa_webserver_host = local.mwaa_webserver_url == null ? null : trimsuffix(
    trimprefix(local.mwaa_webserver_url, "https://"),
    "/"
  )
}
...
resource "aws_ssm_parameter" "mwaa_webserver_host" {
  count     = local.mwaa_webserver_host == null ? 0 : 1
