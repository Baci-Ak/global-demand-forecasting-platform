"""
DAG: 22_predict_next_28_days_ecs

Purpose
-------
Run production batch forecasting on ECS, orchestrated by MWAA.
...
3) Override the container command to run: predict-next-28-days
...
# Daily production batch scoring.
# Runs after the weekly retraining window on Mondays and refreshes the rolling
# 28-day forecast horizon for downstream warehouse consumers.
DAG_SCHEDULE = "0 4 * * *"
...
with DAG(
    dag_id="22_predict_next_28_days_ecs",
    description="Production next-28-days batch forecasting on ECS",
    start_date=DAG_START_DATE,
    schedule=DAG_SCHEDULE,
    catchup=False,
    tags=["ml", "prediction", "forecast", "ecs", "lightgbm"],
    ...
    predict_next_28_days_ecs = EcsRunTaskOperator(
        task_id="predict_next_28_days_on_ecs",
    ...
    runtime_config >> predict_next_28_days_ecs