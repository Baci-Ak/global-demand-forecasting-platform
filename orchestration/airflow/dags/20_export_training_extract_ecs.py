"""
DAG: 20_export_training_extract_ecs

Purpose
-------
Export the production ML training extract from Redshift to S3 using ECS,
...
3) Override the container command to run: export-training-extract
...
DAG_SCHEDULE = "0 2 * * 1"  # Weekly Monday at 02:00 London time
DEFAULT_ARGS = default_args(retries=1)


# ------------------------------------------------------------------------------
# Runtime discovery contract
# - Keep parse-time safe: define parameter names only here.
# - Resolve actual values inside a task at runtime.
# ------------------------------------------------------------------------------
...
with DAG(
    dag_id="20_export_training_extract_ecs",
    description="Production training extract export on ECS",
    start_date=DAG_START_DATE,
    schedule=DAG_SCHEDULE,
    catchup=False,
    tags=["ml", "extract", "ecs", "redshift"],
    ...
    # --------------------------------------------------------------------------
    # Task: Resolve ECS runtime configuration from SSM
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # Task: Export training extract on ECS
    # --------------------------------------------------------------------------
    export_training_extract_ecs = EcsRunTaskOperator(
        task_id="export_training_extract_on_ecs",
        ...
        overrides={
            "containerOverrides": [
                {
                    "name": "ml-runtime",
                    "command": ["export-training-extract"],
                    "environment": [
    ...
    runtime_config >> export_training_extract_ecs